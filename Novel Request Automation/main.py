...
"""
    ...
    for index, row in df.iterrows():
        ...
        border_color_hex = "#f59e0b" # Default Yellow
        badge_class = "status-pending"
        badge_text = "Pending"
        action_html = ""
        
        # [FEATURE 2] Tooltip Text Generate karo
        tooltip_text = f"Download {novel_name} by {author_name}"
        ...
        if "fulfilled" in status_lower:
            ...
            # Agar link available hai
            if download_link and download_link.lower() != "nan":
                # [FEATURE 1] Check agar YouTube link hai
                if "youtube.com" in download_link.lower() or "youtu.be" in download_link.lower():
                    # YouTube Style (Red Button)
                    tooltip_text = f"Watch {novel_name} by {author_name}" # Update tooltip for video
                    action_html = f'<a href="{download_link}" class="download-btn" style="background: #ef4444;" target="_blank" rel="noopener noreferrer" title="{tooltip_text}"><i class="fab fa-youtube"></i> Watch on YouTube</a>'
                else:
                    # Normal PDF Style (Green Button)
                    action_html = f'<a href="{download_link}" class="download-btn" target="_blank" rel="noopener noreferrer" title="{tooltip_text}">Download PDF</a>'
            else:
                action_html = '<p class="message-text" style="color:#047857;">Uploaded (Link coming soon)</p>'

        # 2. Error / Not Found Logic
        ...
        # 3. Pending / App Logic
            ...
            # Agar pending me bhi koi link hai (Jaise App Link)
            if download_link and download_link.lower() != "nan":
                btn_text = "Open Link"
                if "app" in status_lower:
                    btn_text = "Get App"
                
                # Check YouTube here too (Just in case)
                if "youtube.com" in download_link.lower() or "youtu.be" in download_link.lower():
                     action_html += f'<a href="{download_link}" class="download-btn" style="background:#ef4444; margin-top:5px;" target="_blank" rel="nofollow" title="Watch {novel_name} by {author_name}"><i class="fab fa-youtube"></i> Watch Video</a>'
                else:
                     action_html += f'<a href="{download_link}" class="download-btn" style="background:#f59e0b; margin-top:5px;" target="_blank" rel="nofollow" title="{tooltip_text}">{btn_text}</a>'
    ...
    print(f"🎉 Updated HTML Generated successfully at: {output_file}")


# 👇 APNA FILE PATH YAHAN CHECK KAREIN
excel_input_path = r"C:\Users\PCS\Downloads\Untitled spreadsheet (1).xlsx"
