...
def generate_html_file():
    ...
    # 1. سب سے پہلے مصنف کا نام تلاش کریں 
    author_name = "Unknown Writer"
    for url in urls:
        temp_title = clean_text_from_url(url)
        if " By " in temp_title:
            parts = temp_title.split(" By ")
            if len(parts) > 1:
                name_part = parts[1].strip()
                # نام کے آخر سے 'Pdf' ہٹا دیں تاکہ ہمیں اصل نام مل سکے
                if name_part.lower().endswith(" pdf"):
                    name_part = name_part[:-4].strip()
                
                # اگر مصنف کا نام موجود ہے (خالی نہیں ہے)
                if name_part: 
                    author_name = name_part
                    break # نام مل گیا، لوپ روک دیں

    # 2. تمام لنکس سے ڈیٹا نکالنا اور نامکمل ناموں کو ٹھیک کرنا
    books_data = [] 
    for url in urls:
        book_title = clean_text_from_url(url)
        
        # اگر ٹائٹل کے آخر میں صرف " By" ہے اور نام کٹ گیا ہے، تو محفوظ کیا گیا نام آگے لگا دیں
        if book_title.endswith(" By"):
            book_title = book_title + " " + author_name
            
        # ٹائٹل اور لنک کو عارضی طور پر محفوظ کر لیں
        books_data.append((book_title, url))

    # 3. چھانٹی (Alphabetical Order)
    books_data.sort(key=lambda x: x[0])

    # 4. ترتیب شدہ ڈیٹا سے HTML بنانا
    links_list = []
