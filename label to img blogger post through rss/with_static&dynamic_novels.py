#good but without error handling 3 novels in one row
import requests
import json
import os
import re
...
output_file_path = os.path.join(downloads_path, "hybrid_library_final_fixed.txt")
...
print(f"Generating Fixed Hybrid Code for: {LABEL_URL}")
label_name = get_clean_label(LABEL_URL)

# RSS Fetch for Static Data (Python side)
...
try:
    ...
    for item in root.findall('channel/item'):
        ...
        static_novels_list.append({
            "display_title": clean_title(title_raw),
            "tooltip_title": title_raw.strip(),
            "link": link,
            "img": optimize_img(img_url)
        })

    static_novels_list.sort(key=lambda x: x['display_title'])
    json_data = json.dumps(static_novels_list, ensure_ascii=False)
...
.novel-title {{ padding: 8px 5px; font-size: 12px; font-weight: 700; color: #333; text-align: center; line-height: 1.3; height: 45px; display: flex; align-items: center; justify-content: center; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; }}

/* --- MOBILE OPTIMIZATION (3 Novels Per Row) --- */
...
function renderStatic() {{
    ...
    if (renderedStaticCount >= staticNovels.length) {{
        ...
        btn.innerHTML = "Load Latest Online Novels";
    }}
}}
...
function startDynamicFetch() {{
    ...
    script.src = "https://www.urdunovelbanks.com/feeds/posts/default/-/{label_name}?alt=json-in-script&callback=feedCallback&max-results=500";
...
window.feedCallback = function(json) {{
    ...
    entries.sort(function(a, b) {{
        var dateA = new Date(a.published.$t).getTime();
        var dateB = new Date(b.published.$t).getTime();
        return dateB - dateA;
    }});
    dynamicData = entries;
    currentMode = "DYNAMIC_PHASE"; 
    btn.disabled = false;
...
function createCard(displayTitle, link, img, idx, tooltipText) {{
    ...
    print(f"Success! Mobile-friendly code generated: {output_file_path}")
...
#good but without error handling
...
#         static_novels_list.append({
#             "display_title": clean_title(title_raw),
#             "tooltip_title": title_raw.strip(), # Hover k liye raw title
#             "link": link,
#             "img": optimize_img(img_url)
#         })

#     # Static data always sorted A-Z for Library Look
#     static_novels_list.sort(key=lambda x: x['display_title'])
#     json_data = json.dumps(static_novels_list, ensure_ascii=False)
...
#     }}
#     var script = document.createElement('script');
#     // Using json-in-script for faster loading
#     script.src = "https://www.urdunovelbanks.com/feeds/posts/default/-/{label_name}?alt=json-in-script&callback=feedCallback&max-results=500";
#     document.body.appendChild(script);
# }}
...
#     // --- CRITICAL SORTING LOGIC: LATEST TO OLDEST ---
#     entries.sort(function(a, b) {{
#         var dateA = new Date(a.published.$t).getTime();
#         var dateB = new Date(b.published.$t).getTime();
...
#     a.className = 'novel-card';
#     a.href = link;
#     a.title = tooltipText.replace(/"/g, '&quot;'); // Poora title tooltip mein
...
#     print(f"Success! Final Optimized Code with Correct Sorting: {output_file_path}")
