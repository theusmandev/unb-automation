...
"""
╔══════════════════════════════════════════════════════════════╗
║     Unicode Urdu Novel -> InPage Converter  |  FINAL v6     ║
║                                                              ║
║  Handles:                                                    ║
║  ✓ Urdu text (complete alphabet + diacritics)               ║
║  ✓ English text RTL fix — brackets/spaces preserved ✅      ║
...
║  ✓ English inside brackets e.g. (QueensMaker()) ✅          ║
...
╚══════════════════════════════════════════════════════════════╝
  ...
  CHANGES in v4 vs v3:
  ─────────────────────────────────────────────────────────────
  fix_english_for_rtl() function rewritten:

  OLD LOGIC (v3) — BROKEN:
    English segment ko word-by-word reverse karta tha.
    "Queens Maker()"  ->  "()Maker Queens"   ✗  (brackets alag, space missing)
    "QueensMaker()"   ->  "()QueensMaker"    ✗  (brackets ulti jagah)

  NEW LOGIC (v4) — FIXED:
    Brackets/punctuation bhi English segment ka hissa samjhe jate hain.
    Poora English segment character-by-character reverse hota hai.
    InPage ka RTL engine isko dobara reverse karke sahi dikhata hai.

    "Queens Maker()"  ->  ")(rekaM sneeuQ"  -> InPage shows: "Queens Maker()" ✅
    "QueensMaker()"   ->  ")(rekaMsneeuQ"   -> InPage shows: "QueensMaker()"  ✅
    "Congrats Alia!"  ->  "!ailA stargnoC"  -> InPage shows: "Congrats Alia!" ✅
  ...
  USE:
      ...
      3. python urdu_novel_to_inpage_FINAL_v4.py
"""
...
# Characters jo English segment ka hissa hain
# (brackets, punctuation sab included taake alag na hon)
_ENG_CHARS = set(
...
# ══════════════════════════════════════════════════════════════
#  STEP 2: REMOVED — English RTL fix (v6)
#
#  InPage ka apna Unicode BiDi engine hai — English text ko
#  automatically sahi direction mein render karta hai.
#  Humara reverse karna galat tha:
#    "Opening"  ->  "gninepO"  ✗
#    "LogDaily" ->  "yliadgoL"  ✗
#
#  FIX: English text bilkul as-is choro. InPage khud sambhalta hai.
# ══════════════════════════════════════════════════════════════

def fix_english_for_rtl(line: str) -> str:
    return line  # NO-OP: InPage handles English direction itself
...
def _has_english_neighbor(line: str, pos: int) -> bool:
    """
    Check karo ke koi bracket English letter ke adjacent hai ya nahi.
    Agar hai, to bracket ko InPage code mein convert NAHI karna.

    (QueensMaker)  ->  ( aur ) dono English ke saath hain  ->  ASCII rakho
    (urdu text)    ->  Urdu ke saath hain                  ->  InPage map karo
    """
    if pos > 0 and _LATIN_RE.match(line[pos - 1]):
        return True
    if pos < len(line) - 1 and _LATIN_RE.match(line[pos + 1]):
        return True
    return False
...
def convert_line(line: str) -> tuple[str, set]:
    ...
    v5 FIX — Bracket handling:
    ─────────────────────────
    ( aur ) sirf tab InPage codes mein convert hote hain jab woh
    Urdu text ke saath hon. Agar English letter adjacent ho to
    ASCII bracket as-is rakha jata hai — InPage RTL engine
    English bracket ko sahi jagah dikhata hai.

      (QueensMaker)  ->  brackets ASCII rahenge  ->  InPage: (QueensMaker) ✅
      (Urdu text)    ->  brackets InPage mapped   ->  InPage: (اردو متن)   ✅
    """
    unmapped = set()
    ...
    for i, ch in enumerate(line):
        # Brackets: English ke saath hain to ASCII rakho, warna InPage map karo
        if ch in ('(', ')') and _has_english_neighbor(line, i):
            result.append(ch)  # ASCII bracket — InPage RTL handles it correctly
        elif ch in UNICODE_TO_INPAGE:
...
def main():
    print()
    print("╔══════════════════════════════════════════════════════╗")
    print("║    Urdu Novel -> InPage Converter  |  FINAL v6      ║")
    print("╚══════════════════════════════════════════════════════╝")
