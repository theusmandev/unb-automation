...
"""
╔══════════════════════════════════════════════════════════════╗
║     Unicode Urdu Novel -> InPage Converter  |  v4.1     ║
║                                                              ║
║  Handles:                                                    ║
║  ✓ Urdu text (complete alphabet + diacritics)               ║
║  ✓ Arabic text in duas (ه ك ة ى إ etc.)  ← v4 FIX
║  ✓ Fixed [ bug: إ ab sahi ا render hoga ← v4.1 FIX         ║
...
PRE_REPLACEMENTS = [
    ...
    # إ U+0625 is now in UNICODE_TO_INPAGE (was causing '[' bug in v4)
]
...
UNICODE_TO_INPAGE = {
    ...
    # ══ v4: Arabic-specific letters (duas mein aate hain) ══
    ...
    '\u0625': CTRL + '\u0081',  # إ Alef hamza below -> same as ا in Nastaleeq (v4.1 fix)
}
...
def convert_line(line: str) -> tuple[str, set]:
    ...
    # Pre-replacements (multi-char Urdu/Arabic combos)
...
def main():
    print()
    print("╔══════════════════════════════════════════════════════╗")
    print("║    Urdu Novel -> InPage Converter  |  v4.1          ║")
    print("║    v4.1 FIX: إ wala '[' bug fix ho gaya            ║")
    print("╚══════════════════════════════════════════════════════╝")
...
# """
# ╔══════════════════════════════════════════════════════════════╗
# ║    Unicode Urdu Novel -> InPage Converter  |  FINAL v4       ║
# ║                                                              ║
# ║  Handles:                                                    ║
# ║  ✓ Urdu text (complete alphabet + diacritics)                ║
# ║  ✓ Arabic verses/prayers (Auto-Normalizes Arabic to Urdu)    ║
# ║  ✓ English text RTL fix (Congrats Alia! sahi dikhega)        ║
# ║  ✓ Decorative symbols  ✿ • ★ ❤ etc.                          ║
# ║  ✓ Line/separator chars ═ ─ ━ ● ◆ etc.                       ║
# ║  ✓ Smart quotes " " ' '                                      ║
...
# INPUT_PATH  = r"C:\Users\PCS\Downloads\arabic.docx"
# OUTPUT_PATH = r"C:\Users\PCS\Downloads\arabic-done.txt"
...
#     # Invisible BiDi Marks & Spaces (Fixes the ? issue between Arabic words)
#     '\u200E': '', '\u200F': '',
#     '\u202A': '', '\u202B': '', '\u202C': '',
#     '\u202D': '', '\u202E': '',
#     '\u2028': '\n', '\u2029': '\n',
...
#     # EXTRA DIACRITICS FALLBACK
#     '\u0654': CTRL + '\u00BF', # ٔ Hamza Above
#     '\u0655': CTRL + '\u00A3', # ٕ Hamza Below (Mapped to standalone hamza)
# }
...
# # ══════════════════════════════════════════════════════════════
# #  STEP 2: Arabic to Urdu Normalizer
# # ══════════════════════════════════════════════════════════════

# def normalize_arabic(text: str) -> str:
#     """
#     Arabic characters ko standard Urdu characters mein badalta hai
#     taake InPage unhe samajh kar theek se render kare aur '?' na de.
#     Yeh step conversion se bilkul pehle hota hai.
#     """
#     mapping = {
#         '\u0647': '\u06C1',  # Arabic Heh -> Urdu Gol Heh (Fixes اللھم issue)
#         '\u064A': '\u06CC',  # Arabic Yeh -> Urdu Choti Yeh
#         '\u0643': '\u06A9',  # Arabic Kaf -> Urdu Kaf
#         '\u0649': '\u06CC',  # Alef Maksura -> Urdu Choti Yeh
#         '\u0629': '\u06C3',  # Teh Marbuta -> Urdu Teh Marbuta
#         '\u0625': '\u0627',  # Alef with Hamza Below -> Alif
#         '\u0671': '\u0627',  # Alef Wasla -> Alif
#         '\u0640': '',        # Tatweel (Kashida) -> Remove
#     }
#     for arb, urd in mapping.items():
#         text = text.replace(arb, urd)
#     return text


# # ══════════════════════════════════════════════════════════════
# #  STEP 3: English RTL Fix
# # ══════════════════════════════════════════════════════════════
...
# # ══════════════════════════════════════════════════════════════
# #  STEP 4: Core Unicode -> InPage Conversion
# # ══════════════════════════════════════════════════════════════

# def convert_line(line: str) -> tuple[str, set]:
#     """Single line convert karo. Returns (converted, unmapped_chars)"""
    
#     # PEHLA STEP: Arabic ko standard Urdu mein laao
...
#     print()
#     print("╔══════════════════════════════════════════════════════╗")
#     print("║    Urdu Novel -> InPage Converter  |  FINAL v4      ║")
#     print("╚══════════════════════════════════════════════════════╝")
#     print(f"  Input   : {INPUT_PATH}")
#     print(f"  Output  : {OUTPUT_PATH}")
#     print(f"  Symbols : {SYMBOL_ACTION}")
...
#     # ── Convert ─────────────────────────────────────────────
#     print("\n  [3/4] Convert + Arabic/English Fixes apply ho rahay hain...")
#     converted, unmapped, total_chars, unconverted_chars = convert_text(text)
#     pct = round((total_chars - unconverted_chars) / max(total_chars, 1) * 100, 1)
#     print(f"    {pct}% characters successfully convert hue")
